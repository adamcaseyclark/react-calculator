  ...
  it("0 Radians Returns A Value Of 0", () => {
    ...
    cy.get(LocatorConstants.DISPLAYED_VALUE).then(
      ($displayAfterEnteringOne) => {
        cy.wrap($displayAfterEnteringOne).should("have.text", "0");

        cy.get(LocatorConstants.INVERSE_HYPERBOLIC_SINE_KEY).click();

        cy.get(LocatorConstants.DISPLAYED_VALUE).then(
          ($displayAfterSelectingInverseHyperbolicCosineOnZero) => {
            cy.wrap($displayAfterSelectingInverseHyperbolicCosineOnZero).should(
              "have.text",
              "0"
            );
          }

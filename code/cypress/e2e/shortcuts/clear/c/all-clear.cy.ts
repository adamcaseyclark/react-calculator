...
describe("Keyboard Shortcuts Of Calculator", () => {
      ...
      cy.get(LocatorConstants.CLEAR_KEY).then(($clearKeyDisplayedText) => {
        ...
        cy.get(LocatorConstants.DISPLAYED_VALUE).then(
          ($displayAfterTypingKeyboardShortcutLetterC) => {
            assert.equal(
              $displayAfterTypingKeyboardShortcutLetterC.text(),
              "0"
            );
          }
        );
  ...
  it("Shortcut (c) On AC Clears All Variables, No Operator Key Is Selected", () => {
    cy.get(LocatorConstants.CLEAR_KEY).then(
        ...
        cy.get(LocatorConstants.DISPLAYED_VALUE).then(
          ($displayAfterTypingKeyboardShortcutLetterC) => {
            ...
            if (Cypress.env("TESTING_IN_DEBUG_MODE")) {
              cy.get(LocatorConstants.PREVIOUSLY_DISPLAYED_VALUE).then(
                (
                  $previouslyDisplayedValuesAfterTypingKeyboardShortcutLetterCOnAC
                ) => {
                  cy.wrap(
                    $previouslyDisplayedValuesAfterTypingKeyboardShortcutLetterCOnAC
                  ).should(

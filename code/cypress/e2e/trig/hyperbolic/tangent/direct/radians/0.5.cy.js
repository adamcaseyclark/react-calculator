...
describe("Trigonometric Function Keys - Hyperbolic Tangent In Radians", () => {
  ...
  it("0.5 Radians Returns A Value Of 0.4621171572600(1|0974)", () => {
    ...
    cy.get(LocatorConstants.DISPLAYED_VALUE).then(($displayAfterEntering0) => {
      cy.wrap($displayAfterEntering0).should("have.text", "0.5");
      ...
      cy.get(LocatorConstants.DISPLAYED_VALUE).then(
        ($displayAfterSelectingHyperbolicTangentOn0) => {
          assert.match(
            $displayAfterSelectingHyperbolicTangentOn0.text(),
            /0\.46211715726001/
          );
        }
      );

...
describe("Trigonometric Function Keys - Hyperbolic Inverse Tangent In Radians", () => {
  ...
  it("0.5 Radians Returns A Value Of 0.5493061443340548", () => {
    ...
    cy.get(LocatorConstants.DISPLAYED_VALUE).then(
      ($displayAfterEntering0Point5) => {
        cy.wrap($displayAfterEntering0Point5).should("have.text", "0.5");

        cy.get(LocatorConstants.INVERSE_HYPERBOLIC_TANGENT_KEY).click();

        cy.get(LocatorConstants.DISPLAYED_VALUE).then(
          ($displayAfterSelectingInverseHyperbolicTangentOn0Point5) => {
            assert.match(
              $displayAfterSelectingInverseHyperbolicTangentOn0Point5.text(),
              /0\.549306144334055/
            );
          }
        );

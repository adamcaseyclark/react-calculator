...
describe("Exponential Notation (EE) Display - No Values Entered", () => {
    beforeEach(() => {
    ...
    it("Exponents Cannot Be A Float Number", () => {
        ...
        cy.get(LocatorConstants.DISPLAYED_VALUE).then(($displayAfterEntering0e12) => {
            cy.wrap($displayAfterEntering0e12).should("have.text", "0 e 12");

            cy.get(LocatorConstants.DECIMAL_KEY).click();

            cy.get(LocatorConstants.DISPLAYED_VALUE).then(($displayAfterSelectingDecimalKey) => {
                cy.wrap($displayAfterEntering0e12).should("have.text", "0 e 12");
            });
        });
    });
